...
class Landing extends Component {
  ...
  handlechang(event) {
    //Store value of district as user select
    this.setState({ dist: "uk." + event.target.value.toLowerCase() });//to conert district to required format of api
  }

  handleChange(event) {
    //store value of username as user enter
    this.props.edit_user(event.target.value);
  }
  handlechange(event) {
    //store value of password as user enters
    this.props.edit_pass(event.target.value);
  }

  async handleSubmit(event) {
    //function to send user data entered in login page and generate auth id using api calls when user submit.
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        this.props.edit_authid(res.data.access_token);
...
const mapDispatchToProps = dispatch => {
  ...
  return {
    ...
    edit_authid: auth_tokenid => {
      dispatch({ type: "AUTHENTICATEID", auth_tokenid: auth_tokenid });
    },
    edit_nam: nam => {
      dispatch({ type: "CITIESLIST", nam: nam });
    }
  };
};
